...
$sql = explode("|", itemsStore(0,$_POST["order_by"]));
?>
<center>
...
if((count($sql)-1) > 0){
	for($x=0;$x<(count($sql)-1);$x++){
		...
		<div class='rank_borda' style='width:70px; height:38px;'>
			<button id='<?php echo $item_details[21]; ?>' style='margin-top:6px; width:60px; height:26px; font-size:11px; overflow:hidden;' onclick="itemStoreDetails(this);">
			<?php
			if($item_details[23] > 2){
				echo "Bid";
			}else{
