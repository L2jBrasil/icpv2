...
$showVideoList = explode("|", showMyVideoList(1,"id DESC",$reg_inicial.", ".$reg_por_pag,$_SESSION["UsuarioLogin"]));
$sql_qtd_reg = explode("|", showMyVideoList(1,"id DESC",0,$_SESSION["UsuarioLogin"]));
$qtd_total_reg = (count($sql_qtd_reg)-1) == 0 ? 0 : (count($sql_qtd_reg)-1);
$reg_final = $qtd_total_reg < $reg_final ? $qtd_total_reg : $reg_final;
?>
<center>
	Total of <b><?php echo $qtd_total_reg; ?></b> videos. Showing of <?php echo empty($reg_final) ? 0 : $reg_atual; ?> to <?php echo $reg_final; ?>.
</center>
	...
	for($x=0; $x < (count($showVideoList)-1); $x++){
		...
		<div id="sign_up<?php echo $v; ?>">
			...
			filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#959595', endColorstr='#1b1b1b',GradientType=0 ); /* IE6-8 */">
				Sent by: <?php echo $showVideo[2]; ?><span style="float:right; padding-right:8px;"><a class="close sprited" href="#">X</a></span>
			</div>
